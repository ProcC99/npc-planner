...
def probe_done_predicate_has_no_sha_conjunct() -> str:
    source = Path("scripts/hooks/task_id_required.py").read_text()
    suspicious = [
        line for line in source.splitlines() if "done" in line and "sha" in line
    ]
    return f"no_sha_conjunct={not suspicious}"
